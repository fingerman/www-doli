...
else
{
    ...
    if ($id > 0)
    {
		...
		if(isset($_POST['submit']) && $_POST['submit'] == 'SPEICHERN'){
			if($id == $user->id || $user->admin || $user->rights->medical24->kompetenzbereich_write){
				$db->query("INSERT INTO ".MAIN_DB_PREFIX."kompetenzbereich_liste SET userid='" . $id . "', cat1='" . intval($_POST['auswahl_1']) . "', cat2='" . intval($_POST['auswahl_2']) . "'");
			}
                }
		// Entfernen
		if(isset($_POST['delete']) && $_POST['delete'] == 'ENTFERNEN'){
			if($id == $user->id || $user->rights->medical24->kompetenzbereich_write){
				$db->query("DELETE FROM ".MAIN_DB_PREFIX."kompetenzbereich_liste WHERE rowid=" . intval($_POST['delid']) . " AND userid='" . $id . "'");
			}elseif($user->admin){
				$db->query("DELETE FROM ".MAIN_DB_PREFIX."kompetenzbereich_liste WHERE rowid=" . intval($_POST['delid']));
			}
		}
		...
		print '<form action="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'" method="POST" name="savezipcodes" enctype="multipart/form-data">';
	        ...
	        print '<h4>Ihre Bereiche</h4>';
                print '<table class="border tableforfield" width="100%">';
                print ' <tr><td><strong>1.Auswahl</strong></td><td><strong>2.Auswahl</strong></td><td>&nbsp;</td></tr>';
                ...
                $resql = $db->query("	SELECT 	e.rowid AS rowid, 
                                        ...
                                        FROM 	".MAIN_DB_PREFIX."kompetenzbereich_liste e
                                        WHERE 	userid='" . $id . "'");
                while($obj = $db->fetch_object($resql)){
                    print '	<tr><td>' . $obj->cat1 . '</td><td>' . $obj->cat2 . '</td>';
