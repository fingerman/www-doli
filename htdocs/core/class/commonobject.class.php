...
abstract class CommonObject
{
	...
	function formAddObjectLine($dateSelector,$seller,$buyer)
	{
		...
		//Line extrafield
		//require_once DOL_DOCUMENT_ROOT.'/core/class/extrafields.class.php'; //**p**
                require_once DOL_DOCUMENT_ROOT.'/core/class/extrafields2.class.php';
...
/**  **p** accordion
     ...
     * @return string
     */
    function showOptionals2($extrafields, $mode='view', $params=null, $keyprefix='') // showOptionals2
    {
		global $_POST, $conf, $langs;
		...
		if (count($extrafields->attribute_label) > 0)
		{
			...
			$e = 0;
                        $i = 0;         // accordion index für die Felder **p**
			foreach($extrafields->attribute_label as $key=>$label)
			{
				if (is_array($params) && count($params)>0) {
					if (array_key_exists('colspan',$params)) {
				...
				if ($extrafields->attribute_type[$key] == 'separate')
				{
					//$out .= $extrafields->showSeparator($key);  
                                    if($i == 0){
                                        $out .= $extrafields->showSeparator2($key);
                                        $out .= '<div><table class="noborder noshadow" width="100%">';
                                    }elseif($i == count($extrafields->attribute_label)){
                                        $out .= $extrafields->showSeparator2($key);
                                    }else{
                                        $out .= '</table></div>';
                                        $out .= $extrafields->showSeparator2($key);
				...
				else
				{
					...
					$e++;
                                ...
                                if ($extrafields->attribute_type[$key] == 'separate')
                                {
                                    if($i <= count($extrafields->attribute_label)-1){
