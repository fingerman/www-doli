 ...
 * Copyright (C) 2013      C�dric Salvador      <csalvador@gpcsolutions.fr>
		...
		if ($mainmenu == 'companies')
		{
			...
			if (! empty($conf->societe->enabled))
			{
				...
				//$newmenu->add("/societe/index.php?leftmenu=thirdparties", $langs->trans("ThirdParty"), 0, $user->rights->societe->lire, '', $mainmenu, 'thirdparties');
                                $newmenu->add("/societe/index.php?leftmenu=thirdparties", $langs->trans("Kontakt erstellen"), 0, $user->rights->societe->lire, '', $mainmenu, 'thirdparties');  //Überschrift Kontakte anlegen

				if ($user->rights->societe->creer)
				{
					//$newmenu->add("/societe/card.php?action=create", $langs->trans("MenuNewThirdParty"),1);
					$newmenu->add("/societe/card.php?leftmenu=prospects&action=create&type=k", "Kunde",1);
                                        $newmenu->add("/societe/card2.php?leftmenu=prospects&action=create&type=k", "Verordner",1);
					$newmenu->add("/societe/card3.php?leftmenu=customers&action=create&type=k", "Gesundheitsdiensleister",1);
                                        $newmenu->add("/societe/card4.php?leftmenu=suppliers&action=create&type=f", "Lieferant<br />",1);
                                        //$newmenu->add("/societe/card3.php?leftmenu=customers&action=create&type=c", "Betreuer",1);
					if (! $conf->use_javascript_ajax) $newmenu->add("/societe/card.php?action=create&amp;private=1",$langs->trans("MenuNewPrivateIndividual"),1);
				}
			}
                        $newmenu->add("/societe/index.php?leftmenu=thirdparties", $langs->trans("Kontakte suchen"), 0, $user->rights->societe->lire, '', $mainmenu, 'thirdparties');  //Überschrift Kontakte anlegen
                       
			$newmenu->add("/societe/list.php?leftmenu=thirdparties", 'Kunden',1);
                        $newmenu->add("/societe/list2.php?leftmenu=thirdparties", 'Verordner',1);
                        $newmenu->add("/societe/list3.php?leftmenu=thirdparties", 'Gesundheitsdiensleister',1);
                        $newmenu->add("/societe/list4.php?type=f&leftmenu=suppliers", 'Lieferanten<br />',1);
			...
			//$newmenu->add("/contact/list.php?userid=$user->id", $langs->trans("MyContacts"), 1, $user->rights->societe->contact->lire); // eigene Kontakte anzeigen
