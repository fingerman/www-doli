...
$search_categ_cus=trim(GETPOST("search_categ_cus",'int'));$search_categ_cus = $search_categ_cus != "" ? $search_categ_cus : 2; ///Kundenkategorie value 11  **p**
...
if (empty($type) || $type == 'c' || $type == 'p')
{
	if (! empty($conf->categorie->enabled))
	{
		...
		$moreforfilter.=$formother->select_categories('customer',$search_categ_cus,'search_categ_cus',1);  ///dummy
	 	$moreforfilter.='</div>';
                $moreforfilter=""; //// **p** $moreforfilter=""; variable gelöscht
	}
}
