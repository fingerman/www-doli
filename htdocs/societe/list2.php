...
$search_categ_cus=trim(GETPOST("search_categ_cus",'int'));$search_categ_cus = $search_categ_cus != "" ? $search_categ_cus : 3; ///Kundenkategorie value 11
...
$object = new Societe2($db);  // **p** $object = new Societe($db);
...
$companystatic=new Societe2($db);  // $companystatic=new Societe($db); **p**
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && ($search_all != '' || $search_cti != '') && $action != 'list')
{
    ...
    $id = $obj->rowid;
    header("Location: ".DOL_URL_ROOT.'/societe/card.php?socid='.$id); //    header("Location: ".DOL_URL_ROOT.'/societe/card.php?socid='.$id);  **p**
    exit;
}
...
if (empty($type) || $type == 'c' || $type == 'p')
{
	if (! empty($conf->categorie->enabled))
	{
		...
		$moreforfilter.=$formother->select_categories('customer',$search_categ_cus,'search_categ_cus',1);  ///dummy
	 	$moreforfilter.='</div>';
                $moreforfilter=""; //// $moreforfilter=""; variable gelöscht
	}
}
