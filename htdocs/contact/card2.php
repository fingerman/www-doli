...
else
{
    ...
    if ($user->rights->societe->contact->creer)
    {
        if ($action == 'create')
        {
            ...
            // dbb - Funktionen **p** Funktionen
            print '<td colspan="3"><select name="poste" id="title" class="minwidth100" value="'.dol_escape_htmltag(GETPOST("poste",'alpha')?GETPOST("poste",'alpha'):$object->poste).'">';
            $functionlist = array( '', 
                                   'Geschäftsführer', 
                                   'Mitarbeiter');
            for($i = 0; $i < count($functionlist); $i++){
                $selected = $functionlist[$i] == dol_escape_htmltag(GETPOST("poste",'alpha')?GETPOST("poste",'alpha'):$object->poste) ? " selected=\"selected\"" : "";
                print '<option value="' . $functionlist[$i] . '"' . $selected . '>' . $functionlist[$i] . '</option>';
            }
            print '</select></td>';
	    ...
	    //print '<td colspan="3"><input name="poste" id="title" type="text" class="minwidth100" maxlength="80" value="'.(isset($_POST["poste"])?GETPOST("poste"):$object->poste).'"></td></tr>';
            ...
            // dbb - Funktionen **p** Funktion bei Änderung
            print '<td colspan="3"><select name="poste" id="title" class="minwidth100" value="'.dol_escape_htmltag(GETPOST("poste",'alpha')?GETPOST("poste",'alpha'):$object->poste).'">';
            $functionlist = array( '', 
                                   'Betreuer', 
                                   'Pfleger', 
