...
if (empty($reshook))
{
	...
	include DOL_DOCUMENT_ROOT.'/core/actions_lineupdown.inc.php';	// Must be include, not include_once
        ...
        // dbb - autocomplete Verordner update der Extrafields für Verordner **p**
        //if ($action == 'setverordner' && $user->rights->commande->creer)
        if ($action == 'setverordner' && $user->rights->commande->creer && GETPOST('canceled') != $langs->trans('Cancel'))
        {
         $db->query("UPDATE     " . MAIN_DB_PREFIX . "commande_extrafields 
                        SET     verordner='" . $db->escape($_POST['verordner']) . "',
                                verordner_id='" . intval($_POST['verordner_id']) . "' 
                        WHERE   fk_object=" . $id);
        }
	...
	else if ($action == 'add' && $user->rights->commande->creer)
	{
		...
		if (! $error) {
                        $result = $db->query("SELECT verordner,krankenkassen FROM " . MAIN_DB_PREFIX . "societe_extrafields WHERE fk_object=" . $socid);  //**p**
                        $obj = $db->fetch_object($result);

                        $_POST['options_verordner'] = $obj->verordner;
                        $_POST['options_krankenkasse'] = $obj->krankenkassen;
                        $_POST['options_verordner_id'] = $socid;
...
if ($action == 'create' && $user->rights->commande->creer)
{
	...
	if ($object->id > 0) {
		...
		*/
	    ...
	    if (! empty($conf->projet->enabled))
	    {
            ...
            // dbb - societe extrafields **p** Verordner autocomplete
            $result = $db->query("SELECT verordner, verordner_id FROM " . MAIN_DB_PREFIX . "commande_extrafields WHERE fk_object=" . $object->id);
            $obj = $db->fetch_object($result);
            $verordner = $obj->verordner;
            $verordner_id = $obj->verordner_id;
            // Verordner
            if($action == 'edit_verordner'){
                //Verordner Auswahl 
                ...
                $morehtmlref.='<br>Verordner : ';
                $morehtmlref.='<form name="setverordner" action="' . $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&amp;extrafield_id=' . $object->socid . '" method="post">';
                $morehtmlref.=' <input type="hidden" name="token" value="' . $_SESSION['newtoken'] . '">';
                $morehtmlref.=' <input type="hidden" name="action" value="setverordner">';
                $morehtmlref.=' <input type="text" name="verordner" autocomplete="off" value="'.$verordner.'">';
                $morehtmlref.=' ID : ';
                $morehtmlref.=' <input type="text" name="verordner_id" value="' . $verordner_id . '">';
                //$morehtmlref.=' <input type="text" name="verordner_id" value="' . $verordner_id . '" style="display: none">';
                $morehtmlref.=' <input type="submit" class="button" value="' . $langs->trans('Modify') . '">';
                //$morehtmlref.=' <input type="submit" class="button" value="' . $langs->trans('Cancel') . '">';
                $morehtmlref.=' <input type="submit" name="canceled" class="button" value="' . $langs->trans('Cancel') . '">';
                $morehtmlref.='</form>';
                $morehtmlref.='<datalist id="verordner">';                    //Verordner datalist
                $morehtmlref.=$options_verordner;
                $morehtmlref.='</datalist>';
                $morehtmlref.='<script>';
                $morehtmlref.='$("input[name=verordner]").attr("list", "verordner");'; //Verordner Autocomplete 
                $morehtmlref.='$("input[name=verordner]").on("keyup select", function(){';       /// Verordner ID
                $morehtmlref.=' var value=this.value;';                                           //" var value=this.value;\n" .    // this.value=arr[1];\n" . 
                $morehtmlref.=' var arr=value.split(": ");';
                $morehtmlref.=' if(arr.length==2){';
                $morehtmlref.='     $("input[name=verordner]").val(arr[1]);';
                $morehtmlref.='     $("input[name=verordner_id]").val(arr[0]);';
                $morehtmlref.=' } else {';
                $morehtmlref.='     $("input[name=verordner]").val(arr[0]);';
                $morehtmlref.=' }';
                $morehtmlref.='});';

                //$morehtmlref.='$("td[text=Verordner]").hide();';
                $morehtmlref.='</script>';
            }else{
                $morehtmlref.='<br>Verordner <a href="' . DOL_URL_ROOT . '/commande/card.php?action=edit_verordner&amp;id=' . $object->id . '"><img src="' . DOL_URL_ROOT . '/theme/eldy/img/edit.png" alt="" title="' . $langs->trans("Edit") . '" class="pictoedit"></a> : ' . $obj->verordner . ' ID : ' . $obj->verordner_id;
            }
            // ende verordner autocomplete
		...
		*/
		...
		if (! empty($conf->use_javascript_ajax) && $object->statut == Commande::STATUS_DRAFT) {
			include DOL_DOCUMENT_ROOT . '/core/tpl/ajaxrow.tpl.php';
                ...
                // dbb - Hilfsmittelnummer **p** Nummer aus Produkte
                $products = array();
                $result = $db->query("SELECT rowid, fk_product FROM ".MAIN_DB_PREFIX."commandedet WHERE fk_commande=" . $object->id);
                while($obj = $db->fetch_object($resql)){
                    $products[] = array('rowid' => $obj->rowid, 'fk_product' => $obj->fk_product);
                }
                for($i = 0;$i < count($products); $i++){
                    $result = $db->query("SELECT hilfsmittelnummer FROM ".MAIN_DB_PREFIX."product_extrafields WHERE fk_object=" . $products[$i]['fk_product']);
                    $obj = $db->fetch_object($resql);
                    $hilfsmittelnummer = $obj->hilfsmittelnummer;
                    $db->query("UPDATE ".MAIN_DB_PREFIX."commandedet_extrafields SET hilfsmittelnummer='" . $hilfsmittelnummer . "' WHERE fk_object=" . $products[$i]['rowid']);
                }
  ...
  </script>';// accordion ende **p** active false = zugeklappt active: 0 ist der 1 aufgeklappt
                // **p** verordner verstecken
                print '<script>';
		...
		if ($action != 'presend' && $action != 'editline') {
			...
			if (empty($reshook)) {
                                ...
                                if ($object->statut > Commande::STATUS_DRAFT) {
                                    ...
                                    print '<ul id="menu_booking">
 <li><div>Buchen</div>
  <ul>
   <li><div>KV Angebot</div>
    <ul>
     <li><div><a href="' . DOL_URL_ROOT . '/auftrag/card_xxx.php?id=' . $object->id . '" class="linkwhite">Kostenvoranschlag</a></div></li>
     <li><div><a href="' . DOL_URL_ROOT . '/auftrag/card_xxx.php?id=' . $object->id . '" class="linkwhite">Kostenvoranschlag Erinnerung</a></div></li>
     <li><div><a href="' . DOL_URL_ROOT . '/auftrag/card_xxx.php?id=' . $object->id . '" class="linkwhite">Rezeptanforderung</a></div></li>
    </ul>
   </li>
   <li><div><a href="' . DOL_URL_ROOT . '/auftrag/card_xxx.php?id=' . $object->id . '" class="linkwhite">KV-genehmigt / offen</a></div></li>
   <li><div><a href="' . DOL_URL_ROOT . '/auftrag/card_xxx.php?id=' . $object->id . '" class="linkwhite">Auftragsgenehmigung</a></div></li>
   <li><div>Lieferschein</div>
    <ul>
     <li><div><a href="' . DOL_URL_ROOT . '/auftrag/card_xxx.php?id=' . $object->id . '" class="linkwhite">Lieferschein</a></div></li>
     <li><div><a href="' . DOL_URL_ROOT . '/auftrag/card_xxx.php?id=' . $object->id . '" class="linkwhite">Lieferschein mit Unterschriftenpad</a></div></li>
    </ul>
   </li>
   <li><div><a href="' . DOL_URL_ROOT . '/auftrag/card_xxx.php?id=' . $object->id . '" class="linkwhite">Rezept buchen</a></div></li>
  </ul>
 </li>
</ul>
<script>
...
<style>
.ui-menu {width: 200px;}
.ui-menu-item {padding: 6px;text-align: left;background-color: rgb(80, 90, 120);color: #ffffff;}
.ui-state-active a.linkwhite, .ui-state-active a.linkwhite:link, .ui-state-active a.linkwhite:visited {color: #000;}
a.linkwhite:link, a.linkwhite:visited {color: #fff;}
a.linkwhite:hover, a.linkwhite:active {color: #000000;}
