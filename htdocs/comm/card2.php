...
if ($id > 0)
{
    ...
    /*  //**p** Tabs ändern im Headbereich
    $head = array();

        $r = 0;

        $head[$r] = array();
        $head[$r][0] = DOL_URL_ROOT . '/societe/card2.php?socid='.$object->id;
        $head[$r][1] = 'Karte';
        $head[$r][2] = 'card';
        $r++;

        $head[$r] = array();
        $head[$r][0] = DOL_URL_ROOT . '/comm/card2.php?socid='.$object->id;
        $head[$r][1] = 'Lead/Kunde';
        $head[$r][2] = 'customer';
        $r++;

        $head[$r] = array();
        $head[$r][0] = DOL_URL_ROOT . '/societe/consumption2.php?socid='.$object->id;
        $head[$r][1] = 'Verkn&uuml;pfte Eintr&auml;ge';
        $head[$r][2] = 'consumption';
        $r++;

        $head[$r] = array();
        $head[$r][0] = DOL_URL_ROOT. '/societe/rib2.php?socid='.$object->id;
        $head[$r][1] = 'Bank Konten';
        $head[$r][2] = 'rib';
        $r++;

        $head[$r] = array();
        $head[$r][0] = DOL_URL_ROOT . '/societe/note2.php?id='.$object->id;
        $head[$r][1] = 'Hinweise';
        $head[$r][2] = 'note';
        $r++;

        $head[$r] = array();
        $head[$r][0] = DOL_URL_ROOT . '/societe/document2.php?socid='.$object->id;
        $head[$r][1] = 'Verkn&uuml;pfte Dokumente';
        $head[$r][2] = 'document';
        $r++;

        $head[$r] = array();
        $head[$r][0] = DOL_URL_ROOT . '/societe/agenda2.php?socid='.$object->id;
        $head[$r][1] = 'Events';
        $head[$r][2] = 'agenda';
        $r++;
    */
