 ...
 * Copyright (C) 2014-2015  Marcos GarcÃ­a           <marcosgdf@gmail.com>
...
function top_htmlhead($head, $title='', $disablejs=0, $disablehead=0, $arrayofjs='', $arrayofcss='', $disablejmobile=0, $disablenofollow=0)
{
    ...
    {
        print "<head>\n";
        //print "<noscript><meta http-equiv='refresh' content='0;url=kein-javascript.php'></noscript>\n"; // **p** funktioniert nicht im ff
        ...
        if (! $disablejs && ! empty($conf->use_javascript_ajax))
        {
            ...
            if (! defined('DISABLE_BROWSER_NOTIF'))
            {
                ...
                {
                    print '<!-- Includes JS of (brwoser layout = '.$conf->browser->layout.')-->'."\n";
...
function top_menu($head, $title='', $target='', $disablejs=0, $disablehead=0, $arrayofjs='', $arrayofcss='', $morequerystring='', $helppagename='')
{
    ...
    if (empty($conf->dol_hide_topmenu) && (! defined('NOREQUIREMENU') || ! constant('NOREQUIREMENU')))
    {
	    ...
	    if (empty($conf->global->MAIN_HELP_DISABLELINK) && empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER))
	    {
	        ...
	        if (empty($helppagename)) $helppagename='EN:User_documentation|FR:Documentation_utilisateur|ES:DocumentaciÃ³n_usuarios';
...
function left_menu($menu_array_before, $helppagename='', $notused='', $menu_array_after='', $leftmenuwithoutmainarea=0, $title='', $acceptdelayedhtml=0)
{
    ...
    if (empty($conf->dol_hide_leftmenu) && (! defined('NOREQUIREMENU') || ! constant('NOREQUIREMENU')))
    {
        ...
        {
            $doliurl='https://www.medicalmax.de';
    		//local communities
    		if (preg_match('/fr/i',$langs->defaultlang)) $doliurl='https://www.medicalmax.de';
    		if (preg_match('/es/i',$langs->defaultlang)) $doliurl='https://www.medicalmax.de';
    		if (preg_match('/de/i',$langs->defaultlang)) $doliurl='https://www.medicalmax.de';
    		if (preg_match('/it/i',$langs->defaultlang)) $doliurl='https://www.medicalmax.de';
    		if (preg_match('/gr/i',$langs->defaultlang)) $doliurl='https://www.medicalmax.de';

            //$appli=constant('DOL_APPLICATION_TITLE');
            $appli=Medicalmax;        // pierre geändert
             //
             /*
            if (! empty($conf->global->MAIN_APPLICATION_TITLE))
    	    {
    	     ...
    	     */
    	     $appli=$conf->global->MAIN_APPLICATION_TITLE; $doliurl='http://www.medicalmax.de';
